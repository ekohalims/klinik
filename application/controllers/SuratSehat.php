...
class SuratSehat extends BaseController{
	function __construct(){
		parent::__construct();
		$this->load->model("modelCetakSurat");
		$this->isLoggedIn($this->global['idUser'],2,44);
    }

    function index(){
        $this->global['pageTitle'] = "SIMRS - Surat Sehat";
		$this->loadViews("cetaksurat/suratsehat/bodySuratSehat",$this->global,NULL,"cetaksurat/suratsehat/footerSuratSehat");
    }
    ...
    function datatablePasien(){
		...
		foreach ($query->result_array() as $dt) {
			...
			$output['data'][]=array($nomor_urut,$dt['noPasien'],$dt['namaLengkap'],$dt['jenisKelamin'],$this->hitungUmur($dt['tanggalLahir'])." Th",$dt['noHP'],$dt['alamat'],"<a href='".base_url('suratSehat/cetakSurat/'.$encoded)."'><i class='fa fa-print'></i></a>");
			$nomor_urut++;
		}
    ...
    function cetakSurat(){
        $this->global['pageTitle'] = "SIMRS - Cetak Surat Sehat";
        $idPasien = $this->dekripsi($this->uri->segment(3));
        $data['dataPemeriksaan'] = $this->modelCetakSurat->dataPemeriksaan($idPasien);
		$this->loadViews("cetaksurat/suratsehat/bodyCetakSurat",$this->global,$data,"cetaksurat/suratsehat/footerCetakSurat");
    }
    ...
    function previewSurat(){
        ...
        $bulan = substr($noPendaftaran,6,2);
        ...
        $this->load->view("cetaksurat/suratsehat/previewSurat",$data);
    }
}