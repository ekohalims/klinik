<div class="row">
	<div class="col-md-12" style="text-align: right;">
		...
		<a class="btn btn-success cetakResep"><i class="fa fa-print"></i> Cetak Resep</a>
	</div>
</div>
...
<div class="modal fade" id="modalResep" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" style="display: none;">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                <h4 class="modal-title" id="myLargeModalLabel">Resep</h4>
            </div>

            <div class="modal-body" id="modalResepContent">                 
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    $('.cetakResep').on("click",function(){
        var url = "<?php echo base_url('inputTindakanRajal/cetakResepModal'); ?>";
        $('#modalResepContent').load(url,{noPendaftaran : noPendaftaran});
        $('#modalResep').modal('show');
    });

    function tampilkanItemModal(idKategori,search,limitStart){
		var url = "<?php echo base_url('inputTindakanRajal/tampilkanItemObat'); ?>";
		$.ajax({
			method : "POST",
			url : url,
    ...
    $(document).on("click",".hapusResep",function(){
        ...
        var urlHapusResep = "<?php echo base_url('inputTindakanRajal/hapusResep'); ?>";
    ...
    function tampilkanCart(){
    	var urlTampilkanCart = "<?php echo base_url('inputTindakanRajal/tampilkanCartResep'); ?>";
