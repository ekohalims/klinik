...
<script type="text/javascript">
    ...
    $('#simpanDokter').on("click",function(){
        ...
        if(namaLengkap=='' || jenisKelamin=='' || noHP=='' || poliklinik==''){
            ...
            var urlSimpanDokter = "<?php echo base_url('dokter/simpanDokterSQL'); ?>";

            $.ajax({
                method : "POST",
                url : urlSimpanDokter,
                data : {namaLengkap : namaLengkap,jenisKelamin : jenisKelamin, noHP : noHP, noIzinPraktek : noIzinPraktek, poliklinik : poliklinik, alamat : alamat,provinsi : provinsi, kabupaten : kabupaten, kecamatan : kecamatan},
                beforeSend : function(){
                    $('#simpanDokter').prop('disabled',true);
                },
                success : function(){
                	$.Notification.notify('success','top right', 'Berhasil!', 'Berhasil menambah dokter'); 	
                	window.location.replace("<?php echo base_url('dokter'); ?>");
                }
            });
        }
